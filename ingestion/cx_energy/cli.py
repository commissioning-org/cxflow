#!/usr/bin/env python3
"""
CLI for CX Energy Ingestion from Power Automate.
...
Usage:
    ...
    python -m ingestion.cx_energy.cli clear
"""
...
def cmd_ingest(args) -> int:
    ...
    print(f"\n{'='*50}")
    print("CX Energy Ingestion Results")
    print(f"{'='*50}")
...
def cmd_list(args) -> int:
    ...
    print(f"Found {len(items)} items in CX Energy data\n")
    ...
    if args.format == "json":
        ...
        for i, item in enumerate(items[:args.limit]):
            if isinstance(item, dict):
                item_id = item.get("id", item.get("Id", item.get("meter_id", f"item_{i}")))
                print(f"{i+1}. {item_id}")
                if args.verbose:
...
def cmd_clear(args) -> int:
    ...
    if not args.force:
        response = input("Are you sure you want to clear all stored CX Energy items? [y/N] ")
        if response.lower() != "y":
            print("Cancelled")
            return 0
...
def main():
    parser = argparse.ArgumentParser(
        description="CX Energy Ingestion from Power Automate",
