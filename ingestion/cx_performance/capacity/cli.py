#!/usr/bin/env python3
"""
CLI for CX Performance Capacity Ingestion from Power Automate.
...
Usage:
    ...
    python -m ingestion.cx_performance.capacity.cli clear
"""
...
def cmd_ingest(args) -> int:
    ...
    print(f"\n{'='*50}")
    print("CX Performance Capacity Ingestion Results")
    print(f"{'='*50}")
...
def cmd_list(args) -> int:
    ...
    print(f"Found {len(items)} items in CX Performance Capacity data\n")
    ...
    if args.format == "json":
        ...
        for i, item in enumerate(items[:args.limit]):
            if isinstance(item, dict):
                item_id = item.get("id", item.get("Id", item.get("capacity_id", f"item_{i}")))
                print(f"{i+1}. {item_id}")
                if args.verbose:
...
def cmd_clear(args) -> int:
    ...
    if not args.force:
        response = input("Are you sure you want to clear all stored CX Performance Capacity items? [y/N] ")
        if response.lower() != "y":
            print("Cancelled")
            return 0
...
def main():
    parser = argparse.ArgumentParser(
        description="CX Performance Capacity Ingestion from Power Automate",
